...
vig_JAGS_setup <- function(index, model) {
  ...
  inits <- function(chain){
    gen_list <- function(chain = chain){
      list( 
        ...
        col_urb_B = rnorm(2),
        det_urb_B = rnorm(2),
        tau_per_A = rwish(3, diag(2)),
        tau_col_A = rwish(3, diag(2)),
        tau_det_A = rwish(3, diag(2)),
        tau_per_B = rwish(3, diag(2)),
        tau_col_B = rwish(3, diag(2)),
        tau_det_B = rwish(3, diag(2)),
  ...
  tmon <- c("v0", "vig_urb", "vcoy0", "SIF_deer", "SIF_deer_high", "SIF_deer_low",
            "SIF_rab",  "SIF_rab_high", "SIF_rab_low", "a", "da", "psi0",
    "psi_urb", "pmu", "lp", "gmu", "gyr", "gb", "pb", "lpb", "tau_yr_A",
    "sigma_per_A", "rho_per_A", "sigma_col_A", "rho_col_A", "sigma_det_A", "rho_det_A",
    "tau_yr_B", "sigma_per_B", "rho_per_B", "sigma_col_B", "rho_col_B", 
    "sigma_det_B", "rho_det_B", "z")
  ...
  mout <- run.jags( model = model, 
    ...
    adapt = 2000, #10000,
    burnin = 1000, #10000 , 
    sample = ceiling(2000/(detectCores()-2)) ,
    thin = 2,
    summarise = FALSE ,
    plots = FALSE,
    method = "parallel")
